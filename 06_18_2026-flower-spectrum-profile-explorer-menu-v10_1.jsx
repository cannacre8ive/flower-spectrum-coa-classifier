...
function ProfileDetail({ profile, strains, selectedStrain, onPickStrain, onGenerateLabel }) {
      ...
      <div style={{ padding:"22px 26px", borderBottom:"1px solid var(--border)" }}>
        <div style={{ display:"flex", alignItems:"baseline", justifyContent:"space-between", marginBottom: 16 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap: 8 }}>
            ...
            <button onClick={() => onGenerateLabel && onGenerateLabel([strain], strain.name)} style={{
              padding:"9px 15px", border:"none", borderRadius: 8, cursor:"pointer",
              background:"var(--accent)", color:"#0e0e0c", fontFamily:M, fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform:"uppercase", whiteSpace:"nowrap",
            }}>↓ Generate Avery 6464 PDF</button>
          </div>
        </div>
...
function DataPanel({ strains, scope, onEdit, onGenerateLabel, onExport, sortKey, setSortKey, groupBy, setGroupBy, showKey, setShowKey, showGrouping }) {
  return (
    <div style={{ border:"1px solid var(--border)", borderRadius: 12, background:"var(--surface)", marginBottom: 22, overflow:"hidden" }}>
      <div style={{ padding:"14px 18px", borderBottom:"1px solid var(--border)", display:"flex", flexWrap:"wrap", gap: 14, alignItems:"center" }}>
        ...
        <div style={{ marginLeft:"auto", display:"flex", gap: 8 }}>
          <button onClick={() => onGenerateLabel && onGenerateLabel(strains, scope)} style={btnStyle}>↓ Label sheet</button>
          ...
          <tbody>
            {strains.map((s, i) => (
              <tr key={i} style={{ borderBottom:"1px solid var(--border)" }}>
                ...
                <td style={tdStyle}>
                  <button onClick={() => onGenerateLabel && onGenerateLabel([s], s.name)} title="Generate Avery 6464 PDF for this strain" style={{
                    padding:"5px 9px", border:"1px solid var(--border)", borderRadius: 5, cursor:"pointer",
                    background:"transparent", color:"var(--accent)", fontFamily:M, fontSize: 10, fontWeight: 700, letterSpacing: 0.5, textTransform:"uppercase", whiteSpace:"nowrap",
                  }}>↓ PDF</button>
                </td>
              </tr>
            ))}
...
// ═══════════════════════════════════════════════════════════════════════════
//  v10.1 · ONE-CLICK CLASSIFIED → AVERY 6464 PDF
//  ───────────────────────────────────────────────────────────────────────────
//  The Flower Spectrum ENGINE above (classify / bandSegments / blendName) is
//  reused as-is — there is no second engine here. The block below is ONLY the
//  jsPDF *drawing* layer, ported verbatim from the standalone Phase-1 label
//  generator (06_16_2026-flower-spectrum-label-pdf-generator-v1_0.html).
//
//  KNOWN DEBT (named, not hidden):
//   1. drawLabel/buildDoc duplicate the draw logic in the standalone HTML tool.
//      The two should be lifted into one shared module in a later pass; until
//      then, any label-design change must be made in BOTH files.
//   2. jsPDF core fonts (Helvetica/Times) carry no Greek glyphs, so α/β/Δ/D-/
//      trans- prefixes are stripped on labels (cleanTerp). The on-screen
//      previews and staff materials keep the full terpene forms.
// ═══════════════════════════════════════════════════════════════════════════

// Lazy-load jsPDF UMD from CDN (single tag, shared). Returns readiness flag.
function useJsPdf() {
  const [ready, setReady] = useState(() => typeof window !== "undefined" && !!window.jspdf);
  useEffect(() => {
    if (typeof window === "undefined" || window.jspdf) { if (window.jspdf) setReady(true); return; }
    const ID = "jspdf-umd-cdn";
    const mark = () => setReady(!!window.jspdf);
    let el = document.getElementById(ID);
    if (el) { el.addEventListener("load", mark); return () => el.removeEventListener("load", mark); }
...
// ── AVERY TEMPLATE REGISTRY (extensible: add new label stocks here) ──
const TEMPLATES = {
  avery6464: {
    id:"avery6464", name:"Avery 6464 · 4\" × 3⅓\" · 6-up", page:"letter",
    pw:8.5, ph:11, lw:4.0, lh:3.333, cols:2, rows:3,
    xs:[0.156, 4.343], ys:[0.500, 3.833, 7.167],
  },
};
...
function cleanTerp(label){
  return label.replace(/^(α|β|γ|δ|Δ|trans|cis|D|L)\s*-\s*/i,'').replace(/^\d+\s*-\s*/,'').trim();
}
...
function drawFingerprintPdf(doc, cx, cy, R, classification){
...
// fit a name to one line (shrink), else wrap to 2 lines
...
function drawLabel(doc, s, lx, ly, T, opts){
  ...
  const blend=blendName(c)+(c.confidence!=='Defined'?'   ·   '+c.confidence:'');
  ...
  doc.text(s.grower, x, afterName);
  // lineage
  doc.setFont('helvetica','normal'); doc.setFontSize(6.5); doc.setTextColor(138,129,112);
  doc.text((s.lineage||'').toUpperCase(), x, afterName+0.15, {maxWidth:lw-bandW-0.45});
  // fingerprint top-right
  drawFingerprintPdf(doc, rgt-fpR, ly+0.5+fpR*0.4, fpR, c);
  // breakdown bars (top 4)
  let by=Math.max(afterName+0.42, ly+1.35); const barX=x, barW=1.5;
  c.ranked.slice(0,4).forEach(r=>{
    ...
    doc.setTextColor(120,114,100); doc.text(r.pct+'%', rgt, by-0.008, {align:'right'});
    by+=0.155;
  });
  // aroma sentence
  doc.setFont('times','italic'); doc.setFontSize(9.5); doc.setTextColor(58,53,48);
  const aromaLines=doc.splitTextToSize(s.aroma, lw-bandW-0.45);
  ...
  doc.text('TOP TERPS · '+topTerpsLabeled(s.values,4).join(' · '), x, fy-0.46, {maxWidth:lw-bandW-0.45});
  doc.setFont('helvetica','bold'); doc.setFontSize(8.5); doc.setTextColor(21,19,15);
  doc.text('THC '+s.thc+'%', x, fy-0.24);
  doc.text('TERPS '+c.totalTerp.toFixed(2)+'%', x+0.92, fy-0.24);
  doc.setFont('times','bolditalic'); doc.setFontSize(15); doc.setTextColor(21,19,15);
  doc.text('$'+s.price.eighth, rgt, fy-0.21, {align:'right'});
  doc.setFont('helvetica','normal'); doc.setFontSize(5.6); doc.setTextColor(140,129,112);
  doc.text('AROMA CLASSIFICATION · NOT EFFECTS OR MEDICAL CLAIMS', x, fy-0.1);
  // cut guide
  if(opts.cutGuides){ doc.setDrawColor(190,190,190); doc.setLineWidth(0.003); doc.rect(lx,ly,lw,lh); }
}

// build the full document from a fill-list of strains
function buildDoc(jsPDFctor, list, templateId, opts){
  const T = TEMPLATES[templateId];
  const doc = new jsPDFctor({ unit:'in', format:T.page });
  const perPage = T.cols*T.rows;
  list.forEach((s, i)=>{
    if(i>0 && i%perPage===0) doc.addPage();
    const cell = i%perPage;
    const rr = Math.floor(cell/T.cols), cc = cell%T.cols;
    drawLabel(doc, s, T.xs[cc], T.ys[rr], T, opts);
  });
  return doc;
}

// Expand a strain set into a fill-list honoring copies + order.
function expandList(strains, copies, order){
  const list = [];
  if(order === "interleaved"){ for(let c=0;c<copies;c++) strains.forEach(s=>list.push(s)); }
  else { strains.forEach(s=>{ for(let c=0;c<copies;c++) list.push(s); }); }
  return list;
}

// One-click label modal: opens with a job ({strains,title,defaultCopies}),
// generates the Avery 6464 PDF, previews it (best-effort) and exposes a
// sandbox-safe data-URI download.
function LabelModal({ job, ready, onClose }) {
  const [copies, setCopies] = useState(6);
  const [cutGuides, setCutGuides] = useState(true);
  const [order, setOrder] = useState("grouped");
  const [out, setOut] = useState(null);
  const [err, setErr] = useState(null);

  useEffect(() => { if (job) { setCopies(job.defaultCopies ?? 6); setOut(null); setErr(null); } }, [job]);

  useEffect(() => {
    if (!job || !ready) return;
    try {
      const { jsPDF } = window.jspdf;
      const cp = Math.max(1, Math.min(500, copies || 1));
      const list = expandList(job.strains, cp, order);
      const doc = buildDoc(jsPDF, list, "avery6464", { cutGuides });
      const T = TEMPLATES.avery6464;
      const pages = Math.max(1, Math.ceil(list.length / (T.cols * T.rows)));
      let blob = null; try { blob = doc.output("bloburl"); } catch (e) { blob = null; }
      const uri = doc.output("datauristring");
      setOut({ uri, blob, count: list.length, pages });
      setErr(null);
    } catch (e) { setErr(String(e && e.message || e)); setOut(null); }
  }, [job, ready, copies, cutGuides, order]);

  if (!job) return null;
  const multi = job.strains.length > 1;
  const fileName = `flower-spectrum-avery6464-${(job.title || "labels").toLowerCase().replace(/[^a-z0-9]+/g, "-")}-${BUILD}.pdf`;

  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.72)", display:"flex", alignItems:"center", justifyContent:"center", zIndex: 300, padding: 20 }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:"var(--surface)", border:"1px solid var(--border)", borderRadius: 14, width:"min(940px, 100%)", maxHeight:"90vh", overflow:"auto" }}>
        {/* header */}
        <div style={{ padding:"18px 22px", borderBottom:"1px solid var(--border)", display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap: 14 }}>
          <div>
            <div style={{ fontFamily:M, fontSize: 9, color:"var(--accent)", letterSpacing: 2.5, textTransform:"uppercase", marginBottom: 4 }}>Avery 6464 · True-PDF · 6-up</div>
            <div style={{ fontFamily:D, fontSize: 21, fontWeight: 700, color:"var(--fg)", lineHeight: 1.1 }}>{multi ? `${job.strains.length} strains · ${job.title}` : job.title}</div>
            <div style={{ fontFamily:B, fontSize: 12, color:"var(--fg-dim)", marginTop: 3 }}>4″ × 3⅓″ · 6 labels per US Letter sheet · drawn in exact inch coordinates.</div>
          </div>
          <button onClick={onClose} style={{ width: 28, height: 28, borderRadius: 7, border:"1px solid var(--border)", background:"var(--bg)", color:"var(--fg-dim)", cursor:"pointer", fontFamily:M, fontSize: 15, lineHeight: 1, flexShrink: 0 }}>×</button>
        </div>

        <div style={{ display:"grid", gridTemplateColumns:"260px 1fr", gap: 0 }}>
          {/* controls */}
          <div style={{ padding:"18px 20px", borderRight:"1px solid var(--border)", display:"flex", flexDirection:"column", gap: 16 }}>
            <div>
              <label style={{ fontFamily:M, fontSize: 9, color:"var(--muted)", letterSpacing: 1.5, textTransform:"uppercase", display:"block", marginBottom: 6 }}>Copies {multi ? "each" : ""}</label>
              <input type="number" min={1} max={500} value={copies} onChange={e=>setCopies(parseInt(e.target.value)||1)}
                style={{ width:"100%", background:"var(--bg)", border:"1px solid var(--border)", color:"var(--fg)", fontFamily:M, fontSize: 13, padding:"9px 10px", borderRadius: 7 }} />
            </div>
            {multi && (
              <div>
                <label style={{ fontFamily:M, fontSize: 9, color:"var(--muted)", letterSpacing: 1.5, textTransform:"uppercase", display:"block", marginBottom: 6 }}>Order</label>
                <select value={order} onChange={e=>setOrder(e.target.value)} style={{ width:"100%", background:"var(--bg)", border:"1px solid var(--border)", color:"var(--fg)", fontFamily:M, fontSize: 12, padding:"9px 10px", borderRadius: 7 }}>
                  <option value="grouped">Group copies</option>
                  <option value="interleaved">Interleave</option>
                </select>
              </div>
            )}
            <label style={{ display:"flex", alignItems:"center", gap: 8, fontFamily:M, fontSize: 10, color:"var(--fg-dim)", textTransform:"uppercase", letterSpacing: 0.5, cursor:"pointer" }}>
              <input type="checkbox" checked={cutGuides} onChange={e=>setCutGuides(e.target.checked)} /> Cut guides
            </label>

            <div style={{ marginTop: 4, padding:"10px 12px", background:"var(--bg)", border:"1px solid var(--border)", borderRadius: 8, fontFamily:M, fontSize: 10, color:"var(--muted)", lineHeight: 1.6 }}>
              {out ? <>{out.count} label{out.count===1?"":"s"} · {out.pages} page{out.pages===1?"":"s"}</> : ready ? "Building…" : "Loading PDF engine…"}
            </div>

            {out && (
              <a href={out.uri} download={fileName} style={{ display:"block", textAlign:"center", padding:"12px 16px", background:"var(--accent)", color:"#0e0e0c", borderRadius: 8, fontFamily:M, fontSize: 12, fontWeight: 700, letterSpacing: 1, textTransform:"uppercase", textDecoration:"none" }}>↓ Download PDF</a>
            )}
            {err && <div style={{ fontFamily:M, fontSize: 10, color:"#C97A5A", lineHeight: 1.5 }}>Couldn’t build the PDF: {err}</div>}

            <div style={{ fontFamily:B, fontSize: 10.5, color:"var(--muted)", lineHeight: 1.55, marginTop: 4 }}>
              Greek prefixes (α/β/Δ) strip on the printed label — jsPDF core-font limit. Verify die-cut alignment on a plain-paper test print before a production run.
            </div>
          </div>

          {/* preview */}
          <div style={{ padding: 18, background:"#1f1d1a", minHeight: 420, display:"flex", alignItems:"center", justifyContent:"center" }}>
            {out && out.blob ? (
              <iframe title="label preview" src={out.blob} style={{ width:"100%", height: 560, border:"1px solid var(--border)", borderRadius: 8, background:"#333" }} />
            ) : (
              <div style={{ textAlign:"center", fontFamily:M, fontSize: 11, color:"var(--muted)", letterSpacing: 0.5, lineHeight: 1.7, padding: 30 }}>
                {!ready ? "Loading the PDF engine…" : err ? "Preview unavailable." :
                  <>Inline preview isn’t available in this sandbox.<br/>The PDF is built and ready — use <strong style={{ color:"var(--fg-dim)" }}>Download PDF</strong> to open it.</>}
              </div>
            )}
          </div>
        </div>
...
export default function App() {
  ...
  const openLabel = useCallback((strainsArr, title) =>
    setLabelJob({ strains: strainsArr, title, defaultCopies: strainsArr.length > 1 ? 1 : 6 }), []);
  ...
  return (
    <div style={{
      ...
      <LabelModal job={labelJob} ready={jsPdfReady} onClose={() => setLabelJob(null)} />
      ...
      <div style={{ maxWidth: 1080, margin:"0 auto", padding:"0 clamp(16px,4vw,40px) 80px" }}>
        ...
        {view === "explorer" && (
          <div style={{ paddingTop: 28 }}>
            ...
            <ProfileDetail profile={activeProfile} strains={strains} selectedStrain={pickedStrain} onPickStrain={setPickedStrain} onGenerateLabel={openLabel} />
          </div>
        )}
